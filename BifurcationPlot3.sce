...
F=0.6
d=1
Psi=0.01:Psimax/Psistep:Psimax
Phi=0.01:Phimax/Phistep:Phimax
...
Delta=(Phi(z)/F)^2+Phi(z)/F+0.25-2*Psi(y)/d                      ////(d^2)*(0.5*F+Psi)^2-2*(F^2)*Psi*d
if Delta>=0 then

qstab=0.5-(Phi(z)/F)+sqrt(Delta)

Dsup=(((1-qstab)^2)*0.5*d+Psi(y))/(0.5*F+Phi(z))       ///((0.5*F+Psi)*d+-sqrt(Delta))/(F^2)           ///!!!!!!!!!!!!!!!!!!!
...
b11=-0.5*d*(n^2-n1^2)/n^2-Psi(y);
b12=(0.5*F+Phi(z))*(1-alpha*n)*exp(-alpha*n);
b13=-d*(1-n1/n);
b14=0;


b21=0;
b22=-(0.5*F*(n1^2)/(n^2)+0.5*F*alpha*(n1^2)/n+alpha*Phi(z)*n1)*exp(-alpha*n);
b23=-Psi(y);
b24=(F*n1/n+Phi(z))*exp(-alpha*n);
